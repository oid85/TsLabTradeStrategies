...
// Фильтр отсутствует
// 
// Выход - по люстре (для лонгов - самый высокий high - atr * koef, для шортов - самый низкий low + atr * koef)
//
// Управление капиталом - FixLot
...
namespace Centaur.Strategies.VolatilityBreakout.VolatilityBreakoutСhandelier
{
    public class VolatilityBreakoutСhandelier_FixLot : IExternalScript
    {
        ...
        public OptimProperty KoeffAtrCandleier = new OptimProperty(0.5, 0.5, 3, 0.5);
        ...
        {           
            ...
            double koeffAtrCandleier = KoeffAtrCandleier;

            // Индикаторы
            // ATR
            var atrObject = new WealthLabIndicators.Atr() { Period = periodAtr };
            IList<double> atr = atrObject.Execute(security);
            firstValidValue = System.Math.Max(firstValidValue, periodAtr * 3);
            ...
            up = ctx.GetData(@"up", new[] { periodAtr.ToString() }, () => Series.Highest(up, periodAtr));
            down = ctx.GetData(@"down", new[] { periodAtr.ToString() }, () => Series.Lowest(down, periodAtr));

            up = Series.Shift(up, 1);
            down = Series.Shift(down, 1);
			
			firstValidValue = Math.Max(firstValidValue, periodAtr * 2);
            ...
            double trailingCandelier = 0.0; // Трейлинг стоп, расчитанный по люстре
            ...
            for (int bar = firstValidValue; bar < count; bar++) // Пробегаемся по всем свечкам
            {
                ...
                if (LastActivePosition == null) // Если позиции нет
                {
                    ...
                    {
                        startTrailing = security.Bars[bar].High - atr[bar] * koeffAtrCandleier; // Считаем первоначальный стоп
                    ...
                    {
                        startTrailing = security.Bars[bar].Low + atr[bar] * koeffAtrCandleier; // Считаем первоначальный стоп
                ...
                else // Если позиция есть
                {
                    if (LastActivePosition.IsLong)
                    {
                        ...
                        if (bar == entryBar)
                        {
                            ...
                            currentTrailing = startTrailing;
                        }
                        else
                        {
                            // Вычисляем самый высокий High с момента открытия позиции
                            double highestHigh = security.Bars[entryBar].High;
                            for (int i = entryBar; i < bar; i++)
                                if (security.Bars[i].High > highestHigh)
                                    highestHigh = security.Bars[i].High;

                            // Вычисляем стоп по люстре
                            trailingCandelier = highestHigh - atr[bar] * koeffAtrCandleier;

                            // Вычисление текущего стопа
                            currentTrailing = Math.Max(currentTrailing, trailingCandelier);
                        }
                    ...
                    else if (LastActivePosition.IsShort)
                    {
                        ...
                        {
                            // Вычисляем самый низкий Low с момента открытия позиции
                            double lowestLow = security.Bars[entryBar].Low;
                            for (int i = entryBar; i < bar; i++)
                                if (security.Bars[i].Low < lowestLow)
                                    lowestLow = security.Bars[i].Low;

                            // Вычисляем стоп по люстре
                            trailingCandelier = lowestLow + atr[bar] * koeffAtrCandleier;

                            // Вычисление текущего стопа
                            currentTrailing = Math.Min(currentTrailing, trailingCandelier);
                        }
